...
//---------------------------------------------------------------------
describe( `221-String.AfterFirstWord Tests`, function ()
{
	...
	it( `should get no words from empty text`, function ()
	{
		let text = '';
		let delimiters = ' ,.!?;:';
		let words = LQC.String.AfterFirstWord( text, delimiters );
	...
	it( `should get the first word and after the first word`, function ()
	{
		...
		{
			word = LQC.String.FirstWord( text, delimiters );
			LIB_ASSERT.strictEqual( word, words[ index ] );
			text = LQC.String.AfterFirstWord( text, delimiters );
		}
		word = LQC.String.FirstWord( text, delimiters );
